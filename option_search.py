...
def get_oi_snapshot_path(ticker, snapshot_date=None):
    ...
    return os.path.join(
        OI_HISTORY_DIR,
        f"{ticker}_OI_{snapshot_date.strftime('%Y%m%d')}.csv"
    )
...
def build_oi_snapshot(df):
    ...
    snapshot = df[required_cols].copy()
...
def calculate_oi_delta(
    ...
    # --------------------------------------------------------
    # 기존 previous 값 제거 후 merge
    # --------------------------------------------------------

    if "previous_openInterest" in df.columns:

        df = df.drop(
            columns=["previous_openInterest"]
        )
    ...
    # 비교 가능 상태
    #
    # 전체 데이터가 이전 snapshot과 비교되었다는 의미
    # --------------------------------------------------------
    ...
    total_current_oi = float(
        df["openInterest"].sum()
    )

    total_previous_oi = float(
        df["previous_openInterest"].sum()
    )
    ...
    print(
        f"   현재 OI: "
        f"{total_current_oi:,.0f}"
    )

    print(
        f"   이전 OI: "
        f"{total_previous_oi:,.0f}"
    )

...
def build_bar_structure(
    ...
    oi_reference = (
        sorted_table["call_oi"].fillna(0)
        +
        sorted_table["put_oi"].fillna(0)
    ).max()

    volume_reference = (
        sorted_table["call_volume"].fillna(0)
        +
        sorted_table["put_volume"].fillna(0)
    ).max()

    premium_reference = (
        sorted_table["call_premium"].fillna(0)
        +
        sorted_table["put_premium"].fillna(0)
    ).max()
...
def _add_horizontal_bar_labels(
    ax,
    y_positions,
    values,
    side,
    ...
    IMPORTANT:
    y_positions must be the exact Y coordinates
    used by barh().

    PUT:
        barh(y, -put_value)
        label -> x = -put_value - offset

    CALL:
        barh(y, call_value)
        label -> x = call_value + offset
    """
    ...
    for y_pos, value in zip(
        ...
        if (
            not np.isfinite(y_pos)
            or not np.isfinite(value)
            or value <= 0
        ):
            continue

        label = _dashboard_num(
            value
        )
...
def build_dashboard_image(
    ...
    ax.text(
        ...
        (
            f"Spot ${spot:.2f}"
            f"   |   "
            f"Strike ${min_strike:g}–${max_strike:g}"
            f"   |   "
            f"DTE 1–{max_dte}"
        ),
        transform=ax.transAxes,
    ...
    cards = [
        ...
        (
            ...
            _dashboard_num(coi),
            (
                f"{coi_ratio:.1f}% of OI"
                if np.isfinite(coi_ratio)
                else "N/A"
            )
        ),
        ...
        (
            ...
            _dashboard_money(cp),
            (
                f"{cp_ratio:.1f}% of premium"
                if np.isfinite(cp_ratio)
                else "N/A"
            )
        ),
        ...
        (
            ...
            "nearest 10 contracts"
        ),

        (
            "NET GEX",
            _dashboard_money(net_gex),
            "OI-based proxy"
        )
    ...
    if put_wall is not None:
        ...
        ax.text(
            ...
            (
                f"PUT WALL\n"
                f"${px:g}\n"
                f"OI {_dashboard_num(put_wall['put_oi'])}"
            ),
            ha="center",
            va="center",
            fontsize=9,
    ...
    if call_wall is not None:
        ...
        ax.text(
            ...
            (
                f"CALL WALL\n"
                f"${cx:g}\n"
                f"OI {_dashboard_num(call_wall['call_oi'])}"
            ),
            ha="center",
            va="center",
            fontsize=9,
    ...
    if put_wall is not None:
        ...
        wall_text.append(
            f"Put Wall ${float(put_wall['strike']):g}"
        )

    if call_wall is not None:

        wall_text.append(
            f"Call Wall ${float(call_wall['strike']):g}"
        )
    ...
    ax.text(
        ...
        0.20,
        (
            "OI = existing open interest snapshot "
            "| Premium = trading-value proxy "
            "| GEX = OI-based proxy "
            "| Buy/Sell direction cannot be confirmed"
        ),
        transform=ax.transAxes,
        fontsize=7.5,
        color="#6b7280"
    ...
    if (
        ...
        raise RuntimeError(
            "Dashboard image was not created."
        )

    print(
        f"🖼️ option_dashboard.png "

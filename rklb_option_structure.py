...
def build_oi_bar_structure(
    df,
    current_price,
    max_strikes=OI_BAR_MAX_STRIKES,
    bar_width=OI_BAR_WIDTH,
    strike_range_pct=OI_BAR_RANGE_PCT
):
    ...
    pivot["CALL"] = pd.to_numeric(
        pivot["CALL"],
        errors="coerce"
    ).fillna(0)

    pivot["PUT"] = pd.to_numeric(
        pivot["PUT"],
        errors="coerce"
    ).fillna(0)
    ...
    # --------------------------------------------------------
    #
    # 현재가 주변 Strike를 우선적으로 확보하고
    # 그 안에서 OI가 큰 Strike를 선택
    #
...
def format_oi_bar_structure(
    df,
    current_price,
    max_strikes=OI_BAR_MAX_STRIKES
):

    rows = build_oi_bar_structure(
        df,
        current_price,
...
def build_report(
    ...
    # ========================================================
    # 8. CALL / PUT BAR STRUCTURE
    # ========================================================

    oi_bar_text = (
        format_oi_bar_structure(
            df,
            current_price,
            max_strikes=OI_BAR_MAX_STRIKES
        )

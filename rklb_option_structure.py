        ...
        rows.append(
            {
                ...
                "call_gex":
                    call_gex
                    if calls["gex"].notna().any()
                    else np.nan,

                "put_gex":
                    put_gex
                    if puts["gex"].notna().any()
                    else np.nan,
...
# ============================================================
# TODAY EXPIRATION TABLE
# ============================================================

def build_today_expiration_table(
    data,
    today_expiration
):

    if today_expiration is None:

        return pd.DataFrame()

    today = data[
        data["expiration"].astype(str)
        == str(today_expiration)
    ].copy()
...
def build_report(
    ...
    if today_expiration is None:
        ...
        report.append(
            "⚠️ 오늘 만기 옵션을 찾지 못했습니다."
        )

    elif today_table.empty:

        report.append(
            f"⚠️ {today_expiration} 데이터 없음"
        )
        ...
        for _, row in top_today_oi.iterrows():
        ...
        for _, row in top_today_volume.iterrows():
